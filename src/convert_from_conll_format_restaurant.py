...
if len(sys.argv) < 3:
    pred_file = '../output/Restaurant_2017-05-10_14-05-06-906291/000_valid.txt'
    ref_file = '../data/SemEval-2014/Scorer/ref_Restaurant_valid.xml'
else:
...
def map_aspect_to_text(text, current_aspect, current_from, current_to):
    ...
    if current_from <0:
        if text.count(aspects_exploded[0]) > 1:
           offset_bis = offset - 1
           while current_from < 0:
               current_from = text.find(aspects_exploded[0], offset_bis)
               offset_bis -= 1
        ...
        for sentence in sentence_list:
            ...
            if len(aspects) > 0:
                ...
                while i < len(aspects):
                    ...
                    elif fr - 1 == current_to and not((current_aspect == 'food' and asp == 'served') or (current_aspect == 'priced' and asp == 'food')):
                        current_aspect += ' ' + asp
                        current_to = to
                    else:
                        current_aspect, current_from, current_to = map_aspect_to_text(text, current_aspect, current_from, current_to)
                        fp.write('\t\t\t<aspectTerm term="' + replace(current_aspect) + '" from="' + str(current_from) + '" to="' + str(current_to) + '"/>\n')
                        current_aspect, current_from, current_to = asp, fr, to
                    i += 1
